...
export function Button({
    ...
    const baseClasses =
        'flex items-center justify-center cursor-pointer transition disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed';

    const buttonClasses = `${buttonVariants[variant]} ${buttonSizes[size]} ${baseClasses} ${className}`
        .replace(/\s+/g, ' ')
        .trim();
